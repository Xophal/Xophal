-- Add a JSONB snapshot column to test_responses and update submission RPC
ALTER TABLE test_responses ADD COLUMN IF NOT EXISTS question_snapshot JSONB;

-- Replace submit_test_attempt to persist a snapshot of question content/options at submission time
...
BEGIN
  ...
  -- Insert responses and persist a snapshot of the question and options at submission time
  INSERT INTO test_responses (attempt_id, question_id, selected_option_ids, text_answer, numerical_answer, time_spent_seconds, is_bookmarked, is_review_later, question_snapshot)
  SELECT v_attempt.id, r.question_id, r.selected_option_ids, NULLIF(BTRIM(r.text_answer), ''), r.numerical_answer,
         ...
         (SELECT jsonb_build_object(
            ...
            'options', (SELECT jsonb_agg(jsonb_build_object('id', qo.id, 'option_text', qo.option_text, 'is_correct', qo.is_correct) ORDER BY qo.id) FROM question_options qo WHERE qo.question_id = q.id)
         ) FROM questions q WHERE q.id = r.question_id)
  ...
  ON CONFLICT (attempt_id, question_id) DO UPDATE SET
    ...
    time_spent_seconds = EXCLUDED.time_spent_seconds, is_bookmarked = EXCLUDED.is_bookmarked, is_review_later = EXCLUDED.is_review_later, question_snapshot = COALESCE(test_responses.question_snapshot, EXCLUDED.question_snapshot), updated_at = NOW();
  ...
  -- Compute correctness and marks using current keys (trusted at submission time)
